...
export function Sidebar() {
  ...
  return (
    <motion.aside
      ...
      className="fixed left-0 top-0 h-screen bg-gradient-to-b from-[#f7f6f3] via-[#f0f9e8] to-[#e8f5d9] dark:bg-gradient-to-b dark:from-[#f7f6f3] dark:via-[#f0f9e8] dark:to-[#e8f5d9] backdrop-blur-2xl border-r border-[#e5e4df] z-40 flex flex-col transition-colors duration-300"
    >
      {/* Logo */}
      <div className="h-20 flex items-center px-5 border-b border-[#e5e4df]">
          ...
          <AnimatePresence>
            {!isCollapsed && (
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="overflow-hidden">
                <h1 className="font-bold text-[#1a1a2e] text-base whitespace-nowrap">Legal CMS</h1>
            ...
            <NavLink key={item.path} to={item.path}
              className={clsx(
                ...
                isActive
                  ...
                  : 'text-[#1a1a2e] hover:bg-[#efeee9] hover:text-[#1a1a2e]'
              )}
            >
              {isActive && (
      ...
      <button onClick={() => setIsCollapsed(!isCollapsed)}
        className={`absolute -right-3 top-24 w-6 h-6 rounded-full bg-[#1a1a2e] border border-[#1a1a2e] flex items-center justify-center ${colors.hoverBg} hover:border-transparent transition-all shadow-lg group`}
      >
        <motion.div animate={{ rotate: isCollapsed ? 180 : 0 }} transition={{ duration: 0.3 }}>
          <ChevronLeft className={`w-4 h-4 text-white ${user?.role === 'public' ? 'group-hover:text-[#1a1a2e]' : 'group-hover:text-white'}`} />
        </motion.div>
      </button>
