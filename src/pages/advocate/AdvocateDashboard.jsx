    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#6b6b80]">Welcome, {user?.name || 'Advocate'} — Manage your cases, hearings, and evidence</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-[#6b6b80]">{tasks.filter(t => t.status === 'completed' || t.completed).length}/{tasks.length}</span>
            </div>
            <div className="space-y-2">
              {tasks.slice(0, 4).map((task, i) => (
                <motion.div key={task.id} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.05 }}
                  className={`flex items-center gap-3 p-3 rounded-xl ${task.status === 'completed' || task.completed ? 'bg-[#b4f461]/10' : 'bg-[#f7f6f3] dark:bg-[#1a1a2e]'}`}>
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${task.status === 'completed' || task.completed ? 'bg-[#b4f461] border-[#b4f461]' : 'border-[#6b6b80]'}`}>
                    {(task.status === 'completed' || task.completed) && <CheckCircle className="w-3 h-3 text-[#1a1a2e]" />}
                  </div>
                  <span className={`text-sm flex-1 ${task.status === 'completed' || task.completed ? 'text-[#6b6b80] line-through' : 'text-[#1a1a2e] dark:text-white'}`}>{task.title}</span>
                  {task.priority === 'high' && !(task.status === 'completed' || task.completed) && <AlertTriangle className="w-4 h-4 text-amber-500" />}
                </motion.div>
              ))}
            </div>
          </motion.div>
