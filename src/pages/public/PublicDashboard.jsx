    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-400">Track your cases and upcoming hearings</p>
        </div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="relative w-full lg:w-80">
          ...
          <input type="text" value={searchId} onChange={(e) => setSearchId(e.target.value)} placeholder="Search by Citizen ID..."
            className="w-full pl-12 pr-4 py-3 bg-white/[0.03] border border-white/5 rounded-xl text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500/30 transition-all" />
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, i) => (
          <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
            className="relative group p-5 rounded-2xl bg-gradient-to-b from-white/[0.05] to-white/[0.02] border border-white/5 hover:border-white/10 transition-all overflow-hidden"
          >
              ...
              <motion.div key={c.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + i * 0.1 }}
                ...
                className={`p-5 rounded-2xl border cursor-pointer transition-all duration-300 ${
                  selectedCase?.id === c.id 
                    ? 'bg-gradient-to-r from-indigo-500/10 to-purple-500/5 border-indigo-500/30' 
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-white font-medium mb-1 truncate">{c.title}</h3>
            ...
            <div className="space-y-3">
              {notifications.slice(0, 3).map((n, i) => (
                <motion.div key={n.id} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 + i * 0.1 }}
                  className={`p-3 rounded-xl ${n.type === 'urgent' ? 'bg-red-500/10 border border-red-500/20' : 'bg-white/[0.02]'}`}>
