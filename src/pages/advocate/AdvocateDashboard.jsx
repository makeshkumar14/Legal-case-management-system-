    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400">Manage your cases, hearings, and evidence</p>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="grid gap-3">
            {filteredCases.map((c, i) => (
              <motion.div key={c.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + i * 0.08 }}
                className={`p-5 rounded-2xl border-l-4 border border-white/5 hover:border-white/10 transition-all cursor-pointer group ${priorityColors[c.priority] || ''}`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-white font-medium mb-1 truncate group-hover:text-indigo-400 transition-colors">{c.title}</h3>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-2">
              {tasks.slice(0, 4).map((task, i) => (
                <motion.div key={task.id} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.05 }}
                  className={`flex items-center gap-3 p-3 rounded-xl ${task.completed ? 'bg-emerald-500/5' : 'bg-white/[0.02]'}`}>
                  ...
                  <span className={`text-sm flex-1 ${task.completed ? 'text-slate-500 line-through' : 'text-white'}`}>{task.title}</span>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
            className="p-6 rounded-2xl bg-white/[0.02] border border-white/5">
            <h3 className="font-semibold text-white mb-4">Recent Evidence</h3>
